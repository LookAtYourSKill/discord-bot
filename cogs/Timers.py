...
class Timers(commands.Cog):
    ...
    async def create(self, ctx, time=None, *, prize=None):
        ...
        data[str(ctx.guild.id)]["giveaways"]["giveaway_info"]['Giveaway-' + str(giveaway_counter)]["Creator"] = str(ctx.author.id)
        data[str(ctx.guild.id)]["giveaways"]["giveaway_info"]['Giveaway-' + str(giveaway_counter)]["Time"] = giveaway_time.strftime("%m/%d/%Y, %H:%M:%S")
        data[str(ctx.guild.id)]["giveaways"]["giveaway_info"]['Giveaway-' + str(giveaway_counter)]["Prize"] = str(prize)
    ...
    async def reminder(self, ctx, time=None, *, reason='not set'):
        ...
        if time is None:
            ...
            data[str(ctx.guild.id)]["reminders"]["reminder_info"]["Time"] = reminder_time.strftime("%m/%d/%Y, %H:%M:%S")
