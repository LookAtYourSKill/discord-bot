...
class fun(commands.Cog):
    ...
    async def ball(self, ctx, *, question):
        ...
        if data[str(ctx.guild.id)]["Fun"] == 'false':
            embed = discord.Embed(
                description=f'Diese **Extension (Fun) ist momentan deaktiviert!** Wende dich bitte an **den Owner vom Bot** (LookAtYourSkill#8691)',
    ...
    async def slots(self, ctx):
        ...
        if data[str(ctx.guild.id)]["Fun"] == 'false':
            ...
            winembed = discord.Embed(description=f'🎉Du hast in **Slots gewonnen**🎉!\nDu hast {grabbed} gezogen!')

            loseembed = discord.Embed(description=f'Du hast **leider nicht gewonnen**!\nDu hast {grabbed} gezogen!')
    ...
    async def dice(self, ctx):
        ...
        if data[str(ctx.guild.id)]["Fun"] == 'false':
            ...
            embed = discord.Embed(description=f'Der Würfel ist auf **der Nummer {num}** gelandet!')
    ...
    async def rps(self, ctx, name=None):
        ...
        if data[str(ctx.guild.id)]["Fun"] == 'false':
            ...
            if name is None or name not in rps:
                ...
                if bot_move == name:
                    embed = discord.Embed(description=f'{ctx.author.mention} Das ist ein `unentschieden`!')
                    await message.edit(embed=embed)

                elif name == "rock":
                    if bot_move == "paper":
                        embed = discord.Embed(description=f'{ctx.author.mention} Du hast `verloren`')
                        await message.edit(embed=embed)
                    else:
                        embed = discord.Embed(description=f'{ctx.author.mention} Du hast `gewonnen`')
                        await message.edit(embed=embed)
